...
const slugMap = {
  'Acenocumarol 1mg - 4mg':                              'acenocumarol',
  'Apixabana 2,5mg - 5mg':                               'apixabana-25mg-5mg',
  'Cilostazol 50mg - 100mg':                             'cilostazol',
  'Dabigatrana 75mg - 110mg - 150mg':                    'dabigatrana-75mg-110mg-150mg',
  'Dalteparina 2.500 - 5.000 - 10.000 UI':              'dalteparina',
  'Dipiridamol 25mg - 75mg - 100mg':                     'dipiridamol',
  'Edoxabana 15mg - 30mg - 60mg':                        'edoxabana',
  'Fondaparinux 1,5mg - 2,5mg - 5mg - 7,5mg - 10mg':   'fondaparinux',
  'Nadroparina 2.850 - 3.800 - 5.700 - 7.600 - 9.500 UI': 'nadroparina',
  'Prasugrel 5mg - 10mg':                                'prasugrel',
  'Ticagrelor 60mg - 90mg':                              'ticagrelor',
};
...
function renderRespostaRapida(items) {
  if (!items || items.length === 0) return '';
  return items.map(r => {
    const text = typeof r === 'string' ? r : `${r.titulo}: ${r.texto}`;
    return `<span style="background:#e8f4fd;border:1px solid #90CAF9;border-radius:20px;padding:5px 13px;font-size:13.5px;color:#1565C0;font-weight:500;">${text}</span>`;
  }).join('');
}
...
function gerarPagina(m, slug) {
  ...
  const respostaRapidaTags = renderRespostaRapida(m.resposta_rapida);
  ...
  const quandoProcurarBlock = m.quando_procurar_medico
