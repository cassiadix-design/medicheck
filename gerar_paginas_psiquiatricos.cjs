...
const INPUT_PATH = path.join(__dirname, 'psiquiatricos.json');
const MEDS_DIR = path.join(__dirname, 'medicamentos');

const meds = JSON.parse(fs.readFileSync(INPUT_PATH, 'utf8'));
...
function faqAccordion(perguntas) {
  return perguntas.map(p => `
                    <details style="background:#f8f9fa;border:1px solid #dee2e6;border-radius:10px;padding:14px 18px;cursor:pointer;">
                        ...
                        <div style="margin-top:10px;font-size:15px;color:#555;line-height:1.8;">${p.resposta}</div>
                    </details>`).join('\n');
}

function respostaRapidaChips(itens) {
  return itens.map(i =>
    `<span style="background:#e8f4fd;border:1px solid #90CAF9;border-radius:20px;padding:5px 13px;font-size:13.5px;color:#1565C0;font-weight:500;">${i}</span>`
  ).join('');
}

function badgeColor(med) {
  if (med.receita === 'Não') return 'background:#28a745';
  return 'background:#dc3545';
}

function gerarHTML(med, slug) {
  const url = `https://medicheck.med.br/medicamentos/${slug}/`;
  const nomeSimples = med.generico;
...
<head>
    ...
    <title>${nomeSimples}: para que serve, como tomar e efeitos colaterais | MediCheck</title>
    <meta name="description" content="${med.descricao_seo.substring(0, 160)}" />
    <meta name="keywords" content="${nomeSimples}, ${med.nome}, para que serve ${nomeSimples}, efeitos colaterais ${nomeSimples}, bula ${nomeSimples}, receita ${nomeSimples}" />
    <meta property="og:title" content="${nomeSimples}: para que serve, como tomar e efeitos colaterais | MediCheck" />
    <meta property="og:description" content="${med.descricao_seo.substring(0, 160)}" />
    ...
    <script type="application/ld+json">
{
  ...
  "@type": "MedicalWebPage",
  "name": ${JSON.stringify(med.nome)},
  "description": ${JSON.stringify(med.descricao)},
  "url": "${url}",
  "publisher": {
    "@type": "Organization",
    "name": "MediCheck",
...
<script type="application/ld+json">
{
  ...
  "@type": "FAQPage",
  "mainEntity": [
${faqSchema(med.perguntas_frequentes)}
  ]
}
</script>
</head>
<body>
    <div class="container">
        <div class="header">
            ...
            <h1>${nomeSimples}</h1>
            <div class="tipo">${med.tipo}</div>
            <span style="${badgeColor(med)};color:white;padding:6px 14px;border-radius:20px;font-size:14px;font-weight:700;">${med.receita_display}</span>
        </div>
        ...
        <div style="background:linear-gradient(135deg,#e8f4fd,#d0e8f8);border:2px solid #2196F3;border-radius:12px;padding:16px 20px;margin:16px 0 20px;">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px;margin:12px 0;">${respostaRapidaChips(med.resposta_rapida)}</div>
        </div>

            <div class="section-title">💊 O que é e para que serve</div>
            <div class="section-body">${med.descricao}</div>

        <div style="margin-bottom:20px;">
            <div style="font-size:20px;font-weight:700;color:#333;margin-bottom:12px;">💊 Receita Médica</div>
            <div style="padding-left:30px;">
                <span style="${badgeColor(med)};color:white;padding:6px 14px;border-radius:20px;font-size:14px;font-weight:700;">${med.receita_display}</span>
                <p style="font-size:15px;color:#555;margin-top:10px;line-height:1.7;">${med.receita_descricao}</p>
                <p style="font-size:14px;color:#777;margin-top:6px;line-height:1.6;">${med.receita_nota}</p>
            </div>
        </div>

            <div class="section-title">⚠️ Efeitos Colaterais</div>
            <div class="section-body">${med.efeitos}</div>
        ...
        <div style="background:#ffe6e6;border-left:8px solid #dc3545;border-radius:8px;padding:20px;margin:20px 0;font-size:16px;color:#721c24;line-height:1.8;font-weight:600;">
            ⚠️ <strong>Aviso Importante:</strong> ${med.aviso_grave}
        </div>
        ...
        <div style="margin-top:25px;">
            ...
            <div style="background:#d4edda;border-left:8px solid #28a745;border-radius:8px;padding:20px;font-size:16px;color:#155724;line-height:1.8;">
                ${med.recomendacao}<br><br>${med.educacao}
            </div>
        </div>
        ...
        <div style="background:#e8f0fe;border-left:8px solid #3f51b5;border-radius:8px;padding:20px;margin:20px 0;font-size:15px;color:#283593;line-height:1.8;">
            🏥 <strong>Alerta Farmacêutico:</strong> ${med.alerta_farmaceutico}
        </div>

            <div style="margin-top:30px;">
                <div class="section-title">❓ Perguntas Frequentes</div>
                <div style="display:flex;flex-direction:column;gap:12px;">${faqAccordion(med.perguntas_frequentes)}
                </div>
            </div>
...
for (const med of meds) {
  const slug = toSlug(med.generico);
  const dir = path.join(MEDS_DIR, slug);
  ...
  const isRedirect = fs.existsSync(htmlPath) &&
    fs.readFileSync(htmlPath, 'utf8').includes('meta http-equiv="refresh"');

  if (isRedirect) {
    console.log(`⏭  ${slug} — já é redirect, pulando.`);
    continue;
  }

  fs.writeFileSync(htmlPath, gerarHTML(med, slug), 'utf8');
  console.log(`✅ ${slug}`);
  criadas++;
}

console.log(`\n📄 ${criadas} páginas geradas.`);
